...
class domainAccessController {
  ...
  static async add(req, res) {
    try {
        const { domain_name, domain_hosts_id, identifier, type, baseUrl} = req.body.OCXPayload;
        const domainTable = {
  ...
  /**
      ...
      static async find(req, res) {
        try {
            const { domain_name, domain_hosts_id, type, identifier } = req.body.OCXPayload;
      ...
      static async findAll(req, res) {
        try {
         ...
         const responseData = await axios({
          ...
          console.log(responseData, "this is the response data>>>>")
          serverResponse(req, res, 200, {message: 'domain accesses fetched successfully', ...responseData.data});
        } catch (error) {
          return serverError(req, res, 500, { message: error.message });
        }
      }
      ...
      static async remove(req, res) {
        try {
          const { domain_name, domain_hosts_id, type, baseUrl} = req.body.OCXPayload;
          const domainTable = {
                table_name: "domain_access",
                where: [
                  {"column": "domain_name", "operation": "=", "value": domain_name},
