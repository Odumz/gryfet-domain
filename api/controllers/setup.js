...
class Setup {
  ...
  static async Create(req, res) {
    try {
      ...
      // const domainHostsTable = 'help';

      // console.log(createUrl);
      // console.log(domainHostsTable);
      const domainHostsResponse = await Post(createUrl, domainHostsTable);
      // console.log(domainHostsResponse);
      serverResponse(req, res, 200, {
        message: 'setup domain_hosts successfully',
        ...domainHostsResponse,
      });
      const domainAccessResponse = await Post(createUrl, domainAccessTable);
      serverResponse(req, res, 200, {
        message: 'setup domain_access successfully',
  ...
  static async DBSetup(req, res) {
    try {
      ...
      serverResponse(req, res, 200, {
        message: 'setup table domain_hosts successfully',
        ...response,
      });
    } catch (error) {
      return serverError(req, res, 500, { message: error.message });
